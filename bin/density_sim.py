...
from doubling_agent.density_functions import *
...
parser.add_argument('--density', action=StoreAsArray, type=str, nargs='+',
                    help="Parameters to calculate local density and include in state change rates. First parameter is "
                         "the distance form the cell to consider, second parameter is the cell state of interest, the "
                         "third is the transition that is effected." 
                         "Default is distance of 1 from the cell and states 1.",
                    default=[1., 1, 'd2'])
...
params = []
if sim_type == "Density":
    params = ParametersDens(args.params[0], args.params[1], args.params[2], args.params[3],
...
# The parameters defining the impact of the density.

distance = int(args.density[0])
dens_state = [int(args.density[1])]
transition = str(args.density[2])
...
for r in range(repeats):
    ...
    count = 0
    density_update_all(distance, cells, switch_3d, dens_state)
    timing_update_all(cells, params, mot_params, time_step, transition)
    ...
    while count < sim_steps + 1:
        ...
        for cell in sorted_cells:
            ...
            if t < time_step:
                update_cell_diff(cells, cell[0], params, switch_3d, mot_params, transition)
            else:
                break
