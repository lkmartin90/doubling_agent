...
parser.add_argument('--params', action=StoreAsArray, type=float, nargs='+', help="Parameters for the simulation, "
                    ...
                    default=np.array([1., 1., 1., 0.48, 10., 0.1]))
...
params = []
if sim_type == "Steve":
    params = ParametersSteve(args.params[0], args.params[1], args.params[2], args.params[3],
...
for r in range(repeats):
    ...
    # Things are adjusted slightly for this model. There are 3 columns per cell: time to next event, point on the
    # sliding scale of cell type which will take a value between 0 and 1, and "division time". Once a cell has comitted
    ...
    while count < sim_steps + 1:
        ...
        for cell in sorted_cells:
            ...
            if t < time_step:
                update_cell_steve(cells, cell[0], params, switch_3d, mot_params)
            else:
                break
